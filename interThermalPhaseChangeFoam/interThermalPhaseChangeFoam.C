...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
        ...
        Info<< "****" << endl;
        Info<< "****Pressure range: " << max(p) - min(p)
            << " Pa" << endl;
        Info<< "****Max velocity: " << max( mag(U.internalField()) )
            << " m/s" << endl;
        Info<< "****Phase change energy: "
            << sum( phaseChangeModel->Q_pc()*mesh.V() ) << " W" << endl;
        Info<< "****Volume change: "
            << sum( phaseChangeModel->PCV() * mesh.V() ) << " m^3/s" << endl;
